/*
    ...
    goal1: create gridRopeBridge with for each grid-cell a boolean value false (status: done)
    goal2: plot locations of the rope-head on gridRopeBridge (each visited cell: false --> true).
            This is a stepping stone to get to goal 3.
            (status: done)
    goal3: track location of rope-tail.
        To understand where  the rope-tail at any moment resides, I must model how the rope-tail moves.
        To model the new position of the tail, "ropeTailNew" based on a movement from the input data (e.g. input1.txt or input2.txt) I need 2 things:
        a) current location of the tail.    "ropeTailOld" === attribute of object machineToModelKnotsPositionsInGrid.
        b) new location of the head.        "ropeHeadNew" === calculated in fn movefRopeTailOnRopeBridgeOneStepAtATime (for each new 
           instruction from inputfile (e.g. input1.txt or input2.txt)). When location of tailNew has been calculated, ropeHeadNew is saved in 
           object machineToModelKnotsPositionsInGrid as ropeHeadOld, so the updated ropeHeadOld can be used for the next instruction (e.g. U 4). Likewise
           ropeTailNew will be saved in object machineToModelKnotsPositionsInGrid as ropeTailOld. 
        (current location of the head is not needed).

        X = X-coordinate;
        Y = Y-coordinate;

        // pitfall: "<=1", because head can also overlap tail:        
        if(X-headNew - X-tailOld <=1 && Y-headNew - Y-tailOld <= 1 ) {
            //tail is not moving.
        }

        "if(X-headNew - X-tailOld >1 || Y-headNew - Y-tailOld > 1 ){ 
         //pitfall: ||, but not &&, because requirements only allow 1 of the 2 to be true at the same time.
        } else {}
            //tail is moving. 8 options with syntax of the last if-statement: 

            //option1) tail follows head down"south":
            //option2) tail follows head up "north":
            //option3) tail follows head to the right "east":
            //option4) tail follows head to the left "west":
            //option5) tail follows head to "south-east":(in 2 flavors):
            //option6) tail follows head to "north-east":(in 2 flavors):
            //option7) tail follows head to "south-west":(in 2 flavors):
            //option8) tail follows head to "north-west":(in 2 flavors):
...
log(`countOfAllOfThePositionsTheHeadVisitedAtLeastOnce: ${countOfAllOfThePositionsTheHeadVisitedAtLeastOnce}. pitfall: (just start and destination cell of each step)`);
