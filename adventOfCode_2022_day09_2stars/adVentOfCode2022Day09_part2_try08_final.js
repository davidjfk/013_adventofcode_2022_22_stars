/*
     first read comments of previous files.
     2do: create file readThisFirst.md
     status: works: correct result is 2455
*/
        //data with results in console: 
    /*
        nrOfRequiredRowsInGrid: 21  (1-based numbering)
        nrOfRequiredColsInGrid: 26  (1-based numbering)
        cellXAndYCoordinatesOfStartPointInGrid: [15,11] (0-based numbering)
        xCoordinateOfHeadInGrid : destination: 0
        yCoordinateOfHeadInGrid: destination: 0
        Number of times a grid-cell has been set to true (some cells multiple times): 36.
        Answer, a.k.a. countOfAllOfThePositionsRopeTail9VisitedAtLeastOnce: 36.
    ...
    */ 
    ...
    /*
        ...
        xCoordinateOfHeadInGrid : destination: 0
        yCoordinateOfHeadInGrid: destination: 41
        Number of times a grid-cell has been set to true (some cells multiple times): 2615.
        Answer, a.k.a. countOfAllOfThePositionsRopeTail9VisitedAtLeastOnce: 2455.
...
/*
    ...
    D === down === "south" (in a grid).
    L === left === "west" (in a grid).
    R === right === "east" (in a grid).
    U === up === "north" (in a grid).
...
/*
    uc: in input3.txt 36 cells are set to 'tru1'. This variable helps me to quickly jump to any of them in the vsCode-terminal-logging. 
    for the cell where I want to take a look / investigate. But in input2.txt, 2614 times a cell is set to 'tru1'. Some of them are set to true multiple times. 
    the first 'tru1' of the starting cell is not set by this method.
    ex: in inputfile3.txt 36 cells are set to 'tru1', so I can look for e.g. globalCounterSetTailNineToTrue:29
*/
let globalCounterSetTailNineToTrue = 0;
//2do: put this global variable inside my class MachineToModelKnotsPositionsInGrid as an attribute. 



// uc: data to quickly tweek the code: 
...
/* CONFIG: 
    ...
    2 manually set the number of rows and columns and the starting cell, 3 toggles need to take place:
        CONFIG: toggle 1of3:  
        CONFIG: toggle 2of3:  
        CONFIG: toggle 3of3:  
        (use ctrl-f and use as a search term to quickly jump to these 3 places)
    After that you can set ad lib the nr of rows, columns and starting point for a data set.
    pitfall: if nr of rows, columns is too small, or the starting point is set incorrectly, then
    an error wll be thrown. 
 */
...
/* CONFIG: toggle 1of3: 
    uncomment following line of code to: set number of rows "hard-coded": (value at the beginning of this file. 
    comment out the following line of code to: let code calculate minimum required number of rows automatically. 
 */
...
/* CONFIG: toggle 2of3: 
    uncomment following line of code to: set number of cols "hard-coded": (value at the beginning of this file. 
    comment out the following line of code to: let code calculate minimum required number of cols automatically. 
 */
...
/* 
    CONFIG: toggle 1of3:  
    starting point"hard-coded":
    uncomment following line of code to: set starting coordinates (i.e. x, y) "hard-coded": (x and y can be set at the beginning of this file. 
    (or) Comment out the following line of code to: let code calculate minimum required number of cols automatically. 
...
/*
     Table shows input3.txt in the code (with example data from day 9 part 2: https://adventofcode.com/2022/day/9 )
     So this table helps me as a dev tool.
     2do: comment out this table before running big file  input2.txt (with the real data to calculate the day2-result). 
     Reason: table  gets too  big. This  slows down vsCode.
*/
...
/*
 uc: in input3.txt 36 cells are set to 'tru1'. This variable helps me to quickly jump to any of them in the vsCode-terminal-logging. But in
 input2.txt, 2614 times a cell is set to 'tru1'. Some of them are set to true multiple times. 
*/
log(`Number of times a grid-cell has been set to true (some cells multiple times): ${globalCounterSetTailNineToTrue + 1}.  `) 
// Remark: the first 'tru1' of the starting cell is not set by method movefRopeTailOnRopeBridgeOneStepAtATimeForEachInstructionFromInputFile, so I add "+ 1".
...
log(`Answer, a.k.a. countOfAllOfThePositionsRopeTail9VisitedAtLeastOnce: ${countOfAllOfThePositionsRopeTail9VisitedAtLeastOnce}. `);
// correct answer for input3.txt : 36 (ok)
// correct answer for input2.txt: 2455 (ok)
