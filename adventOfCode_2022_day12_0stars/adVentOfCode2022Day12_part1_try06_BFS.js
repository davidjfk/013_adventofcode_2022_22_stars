/*  
    first read: adventOfCode2022Day12_part1_try01.js comment at the top for the bigger picture and defininitions.
        ...
        Use ctrl+f to quickly goto:
            ...
            config_2of2: choose start and destination coordinates (that fit the selected grid at config_1of3)
    ...
    step 6: new problem: code works good with data from input2.txt, but not with input3.txt.
        analysis: assumption: performance issue, because dataset of  input3.txt > input2.txt (adventofcode day 10 
            ...
            conclusion: hypothesis does not make a difference.
            Also for connecting smaller parts in the data grid, performance is a problem. After googling about BFS vs DFS, I think that I should
            be using DFS instead of BFS.

            next step (in next file): depth-first search. 
...
// config_2of2: choose start and destination coordinates (that fit the selected grid at config_1of3)
    // values belonging to commented out grid (right above this comment) (for this grid, configf_1of2 is irrelevant).
    // let start = [0, 0];
...
//   if (grid[r][c].state == 9) return false; 
    ...
    if (grid[r][c].prevState - prevState > 1)  {
        return false
    }
    if (grid[r][c].prevState - prevState < -1)  {
        return false
