...
/*
    ...
    data:
    input.txt  === source file with adventOfCode data to win aventOfCode-star.
...
*/
function isValidNeighborCellInGridHeightMap(gridWithForEachGridCellABooleanWithNameIsVisited, row, col)
// what you do here: compare the current node with the previous current node.
{
    ...
    if (gridWithForEachGridCellABooleanWithNameIsVisited[row][col] == true){
        // heightOfCurrentNode = gridHeightMap[row][col].height;
        return false;
    }

    //
    if (gridWithForEachGridCellABooleanWithNameIsVisited[row][col] == false ){ 
    // if condition is redundant. Skip it.
        ...
        // read carefully: the neighbor (i.e. next) cell of current cell is allowed to be more than 1 lower !!!
        // if ( heightOfCurrentNode - gridHeightMap[row][col].height  > 1){
        //     // neighbor node === neighbor of current node --> but the latter looks messy in the console.log.
        //     log(`neighbor node: letter: ${String.fromCharCode(gridHeightMap[row][col].height +97)}, row: ${row}, col: ${col} `);
        //     log(`neighbor-node is too high: ${gridHeightMap[row][col].height}. So delta is: ${gridHeightMap[row][col].height - heightOfCurrentNode}`);
        //     log(`end: `);
...
// goal: perform DFS traversal on the gridHeightMap:
function DFS(xCoordinateOfStart, yCoordinateOfStart, gridHeightMap, gridWithForEachGridCellABooleanWithNameIsVisited)
{
    /* 
        init  a stack-array, and push the cell, where the DFS must start, into it.
    */
    let stack = [];
    
    // let poppedCell
    stack.push([ xCoordinateOfStart, yCoordinateOfStart ]); // result: e.g. [ [ 1, 2 ] ]
 
    // Iterate until the stack is empty
    while (stack.length!=0) { 
        let currentNode = stack[stack.length-1]; // result: e.g. [ [ 1, 2 ] ]
        ...
        let row = currentNode[0]; // above you have pushed an array into var currentNode. 
        let col = currentNode[1];
   
        // valid === cell is inside the gridHeightMap AND fn DFS in this file has not yet visited this cell.  
        if (!isValidNeighborCellInGridHeightMap(gridWithForEachGridCellABooleanWithNameIsVisited, row, col)){
            continue; // breaks 1 iteration in the while loop.
        }

        // not sure if following line of code is correct:    
        gridWithForEachGridCellABooleanWithNameIsVisited[poppedCell[0]][poppedCell[1]] = false;

        // log(`popped <read: gridHeightMap cell from previous iteration: ${poppedCell}`)
        // log(poppedCell)
        // heightOfCurrentNode = poppedCell.height; 
        
        // if ((gridHeightMap[row][col].height - heightOfCurrentNode > 1 &&  gridWithForEachGridCellABooleanWithNameIsVisited[row][col])){
        //     //log(`nr difference is too big:`)
        //     continue;
        // }
        
        // Mark the current cell as visited:
        gridWithForEachGridCellABooleanWithNameIsVisited[row][col] = true;
 
        // save heightOfCurrentNode to calculate the delta between currentNode and a neighbor node. 
        // 2do: store heightOfCurrentNode as property of object 
        ...
        */
        ...
        // problem a valid cell can be on a branch that is discarded by dfs later on. But
        // this valid cell is not being deducted!!!
        ...
        for (var i = 0; i < 4; i++) {
            var adjx = row + dRow[i];
            var adjy = col + dCol[i];
            stack.push([ adjx, adjy ]); // pushing an adjacent cell on left, right, top and bottom side of the currentNode (currentNode is a cell.)
        }
        //an: during the next run inside the while loop, each of these 4 adjacent cells is a potential currentNode. The direction vectors inside 
        // arrays dRow and dCol (further above where they are initialized) determines the order of the 4. 

        /*
            during the very first iteration (when the [row, col] of start position are pushed  onto the queue), the length of the stack is 1.
            Reason: the for-loop to push all adjacent cells has not yet run. 
        */
...
let gridWithForEachGridCellABooleanWithNameIsVisited = Array.from(Array(ROW), ()=> Array(COL).fill(false));
// log(gridWithForEachGridCellABooleanWithNameIsVisited)

/*
    array gridWithForEachGridCellABooleanWithNameIsVisited looks like this: (each nested array is a row)
    [
  [
    false, false,
    false, false,
...
// fn call: (main fn):
DFS(xCoordinateOfStart, yCoordinateOfStart, gridHeightMap, gridWithForEachGridCellABooleanWithNameIsVisited);
...
log(`lineup of EdgesFromCellStartToCellDestination:`);
log( nrOfEdgesFromCellStartToCellDestination);
/*
177 is wrong.
451 is wrong.
452 is wrong. 
