...
class GameOfKeepAwayPlayedByMonkeys {
    ...
    throwMonkey1() {
        ...
        //    log('monkey 1 has not items, so nothing to do this round.')
            ...
            let itemsWithUpdatedWorryLevel = this.worryLevelOfItemsThatAreControledByMonkey1.map((itemToUpdateWorryLevel) => ((itemToUpdateWorryLevel +6 )  ))
            itemsWithUpdatedWorryLevel.forEach((itemWithUpdatedWorryLevel) => {
    ...
    throwMonkey2() {
        ...
        //    log('monkey 2 has not items, so nothing to do this round.')
            ...
            let itemsWithUpdatedWorryLevel = this.worryLevelOfItemsThatAreControledByMonkey2.map((itemToUpdateWorryLevel) => {
                ...
                let multiplicationOfDivisorsFromAll4Fns = 23*19*13*17;
    ...
    throwMonkey3() {
        if (this.worryLevelOfItemsThatAreControledByMonkey3.length == 0) {
            // log('monkey 3 has not items, so nothing to do this round.')
             ...
             let itemsWithUpdatedWorryLevel = this.worryLevelOfItemsThatAreControledByMonkey3.map((itemToUpdateWorryLevel) => ((itemToUpdateWorryLevel +3 ) ))
             itemsWithUpdatedWorryLevel.forEach((itemWithUpdatedWorryLevel) => {
...
const gameOfKeepAwayPlayedByMonkeys = new GameOfKeepAwayPlayedByMonkeys(day11inputData);
